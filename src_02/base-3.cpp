...
int main ()
{
	...
	uint32_t			deviceCount    = 0;
		...
		// get number of physical devices in system
	...
	std::cout << "Found " << deviceCount << "physical devices" << std::endl;
		...
		// print short info on every device
	for (const auto& device : physDevices ) 
	{
		...
		std::cout << deviceProperties.deviceName 
			  << (deviceProperties.deviceType == VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU ? " discrete" : " embedded" ) 
			  << std::endl;
		...
		std::vector<VkQueueFamilyProperties> queueFamilies ( queueFamilyCount );
			...
			// get actual queue families properties
		vkGetPhysicalDeviceQueueFamilyProperties ( device, &queueFamilyCount, queueFamilies.data () );

		std::cout << "Queue families " << queueFamilyCount << std::endl;

			// print info on every queue family
		for ( const auto& family : queueFamilies )
		{
			std::cout << "\tCount " << family.queueCount;

				// print supported operations for this family
